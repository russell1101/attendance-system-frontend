document.addEventListener("DOMContentLoaded", function () {
                    ...
                    <td class="product_name">
                        <div class="d-flex align-items-center justify-content-center">
                            <div class="flex-grow-1">
                                <h5 class="fs-14 mb-1"><a href="#" class="text-dark">${
                                  product.name
                                }</a></h5>
                            </div>
                        </div>
  ...
  function renderTable() {
    ...
    if (tbody) {
      tbody.innerHTML = productsData
        .map((product) => Components.productRow(product))
        .join("");
    }
  }
  ...
  // 模擬從 DB 撈出來的紀錄 (JOIN product_redemptions, gift_cards, employees)
  const mockHistoryData = [
    {
      code: "GC-20260314-A1B2",
      empName: "王小明",
      prodName: "星巴克 150元飲料券",
      points: 150,
      status: "AVAILABLE",
      exchangeTime: "2026-03-14 10:30",
      usedTime: null,
    },
    {
      code: "GC-20260312-X9Y8",
      empName: "陳大文",
      prodName: "7-11 數位商品券",
      points: 100,
      status: "USED",
      exchangeTime: "2026-03-12 09:15",
      usedTime: "2026-03-13 12:00",
    },
    {
      code: "GC-20251231-Z001",
      empName: "林美麗",
      prodName: "電影票兌換券",
      points: 300,
      status: "EXPIRED",
      exchangeTime: "2025-12-31 15:40",
      usedTime: null,
    },
    {
      code: "GC-20260314-M4N5",
      empName: "張志明",
      prodName: "保溫杯",
      points: 1200,
      status: "AVAILABLE",
      exchangeTime: "2026-03-14 18:20",
      usedTime: null,
    },
  ];
  ...
  const renderHistoryStatus = (status) => {
    switch (status) {
      case "AVAILABLE":
        return '<span class="badge bg-info-subtle text-info">未使用</span>';
      case "USED":
        return '<span class="badge bg-success-subtle text-success">已使用</span>';
      case "EXPIRED":
        return '<span class="badge bg-secondary-subtle text-secondary">已過期</span>';
      default:
  ...
  const renderHistoryTable = () => {
    ...
    if (filteredData.length === 0) {
      historyTableBody.innerHTML = `<tr><td colspan="7" class="text-center py-4 text-muted">查無相符的兌換紀錄</td></tr>`;
      return;
    }
    ...
    historyTableBody.innerHTML = filteredData
      .map(
        (item) => `
            <tr>
                <td><span class="text-muted">${item.exchangeTime}</span></td>
                <td><span class="fw-medium text-primary">${
                  item.code
                }</span></td>
                <td>${item.empName}</td>
                <td>${item.prodName}</td>
                ...
                <td>${renderHistoryStatus(item.status)}</td>
  ...
  if (filterStatusSelect)
    filterStatusSelect.addEventListener("change", renderHistoryTable);
  ...
  if (pointSettingsForm) {
    pointSettingsForm.addEventListener("submit", function (e) {
      ...
      // 取得表單數值
      const onTime = document.getElementById("set-on-time").value;
      const late = document.getElementById("set-late").value;
      const early = document.getElementById("set-early").value;
      const missing = document.getElementById("set-missing").value;
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          // ⚠️ 這裡未來可以放 fetch API 打到後端 Controller
